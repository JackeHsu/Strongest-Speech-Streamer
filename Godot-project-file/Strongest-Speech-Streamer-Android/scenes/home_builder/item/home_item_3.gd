...
var layer # 位于wall或者ground
# 参数
@export var item_size: Vector2i # 物品尺寸
@export var item_layer: int # 物品层数 
@export var interactive_type: String# 互动物品功能
...
func _ready() -> void:
	mouse_enter = false # 进入场景清空mouse_enter的值
	...
	shader_material.shader = preload("res://shaders/sprite_outline.gdshader")
	item.material = shader_material
	item.material.set_shader_parameter("outline_width", 0)
	# 创建碰撞层
	_create_collision()
...
func board(_position: Vector2i): # 边界判断
	var left = Vector2i(_position.x - (item_size / 2).x + 8, _position.y)
	var right = Vector2i(_position.x + (item_size / 2).x, _position.y)
...
func _unhandled_input(event: InputEvent) -> void:
	if event is InputEventMouseButton and event.is_pressed() and event.button_index == MOUSE_BUTTON_LEFT:
		...
		#area_2d.monitoring = false
			...
			elif GameManager.current_state == 0 and !GameManager.hold_item and mouse_enter and event is InputEventMouseButton and event.is_pressed() and event.button_index == MOUSE_BUTTON_LEFT:
				#print(interactive_type)
				if interactive_type == "sleep":
					global.player_sleep.emit()
				elif interactive_type == "game":
					global.play_game.emit()
				elif interactive_type == "book":
					global.read_tips.emit()
				
func _on_area_2d_mouse_entered() -> void: # 用于判断鼠标是否移动至物品上
	if placed and GameManager.hold_item == false and GameManager.current_state == 1:
		mouse_enter = true
	elif placed and GameManager.hold_item == false and GameManager.current_state == 0:
		self.material.set_shader_parameter("outline_width", 2)
		mouse_enter = true

func _on_area_2d_mouse_exited() -> void: # 用于判断鼠标是否离开物品
	mouse_enter = false
	if placed and GameManager.current_state == 0:
