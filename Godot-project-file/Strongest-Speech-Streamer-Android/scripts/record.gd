...
@onready var mic_input_node = $mic_input
...
func upload_speech(): # 上传录音到oss
	if !recording:
		...
		audio_conversion_android() # 安卓平台转换
...
func audio_conversion_android() -> void:
	...
	#var input_audio = "/storage/emulated/0/Documents/my_recording/my_recording" + str(global.player_recording_count) + ".wav"

	var converter = AudioConverter.new()
	var ret = converter.convert_to_mono(input_audio, output_audio)
	if ret == 0:
		print("转换成功！")
...
func audio_conversion() -> void:
	...
	#var ffmpeg_path = get_ffmpeg_path() # 导出时调用此函数
	var ffmpeg_path = "D:/JackeHsu/游戏开发/项目/演讲模拟器/ffmpeg.exe" # 开发时使用此路径
	var input_audio = file_path + "/my_recording/my_recording" + str(global.player_recording_count) + ".wav"
	var output_audio = file_path + "/output_mono.wav"
	#print(input_audio)
...
func _on_mic_input_finished() -> void:
	print("麦克风中断")
