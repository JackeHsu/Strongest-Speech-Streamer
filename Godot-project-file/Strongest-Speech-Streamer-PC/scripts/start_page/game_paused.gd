...
func _on_quit_pressed() -> void: # 存档功能
	...
	# 任务计数
	config.set_value("Task", "tasks", global.tasks_index) 
	config.set_value("Task", "meetings", global.meeting_index) 
	
	# 成就系统
	config.set_value("Achievement", "achievement", AchiManager.achi_get_situation)
	config.set_value("Achievement", "toys", AchiManager.toys) 
	config.set_value("Achievement", "keys", AchiManager.keys) 
	# 场景名称
	config.set_value("SceneName", "scene_name", "main_page")
	...
	# 仓库库存
	if global.home_in == true: # 回过家再保存数据
