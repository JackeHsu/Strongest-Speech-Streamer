extends Panel


@export var item_name: String

var texture_inside: Texture2D = null


func _ready():
	texture_inside = load("res://scenes/home_builder/item/interact_item/bed.png")#load("res://assets/objects/%s.png" % item_name)
	$TextureRect.texture = texture_inside


func _on_Slot_gui_input(event):
	if event is InputEventMouseButton && event.is_pressed() && event.button_index == MOUSE_BUTTON_LEFT:
		GameManager.item_selected.emit(texture_inside)
